...
abstract class AbstractSQL extends AbstractQueryBuilder{
	...
	function create(AbstractDataConnector $data_connection = null){
		/* @var $data_connection \exface\SqlDataConnector\AbstractSqlConnector */
		if (!$data_connection) $data_connection = $this->get_main_object()->get_data_connection();
		...
		$sql = 'INSERT INTO ' . $this->get_main_object()->get_data_address() . ' (' . implode(', ', $columns) . ') VALUES (' . implode('), (' , $values) . ')';
		...
		if ($last_uid_sql_var){
			...
			$last_id = $query->get_last_insert_id();
		}
		$affected_rows = $query->count_affected_rows();
	...
	function update(AbstractDataConnector $data_connection = null){
		...
		foreach ($updates_by_uid as $uid => $row){
			$sql = 'UPDATE ' . $this->build_sql_from() . ' SET ' . implode(', ', $row) . ' WHERE ' . $this->get_main_object()->get_uid_attribute()->get_data_address() . '=' . $uid;
			$query = $data_connection->run_sql($sql);
			$affected_rows += $query->count_affected_rows();
		}
		
		if (count($updates_by_filter) > 0){
			$sql = 'UPDATE ' . $this->build_sql_from() . ' SET ' . implode(', ', $updates_by_filter) . $where;
			$query = $data_connection->run_sql($sql);
			$affected_rows = $query->count_affected_rows();
		}
	...
	function delete(AbstractDataConnector $data_connection = null){
		...
		$sql = 'DELETE FROM ' . $this->build_sql_from() . $where;
		$query = $data_connection->run_sql($sql);
